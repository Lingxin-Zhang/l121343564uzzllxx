...
LONG_STREAM_CACHE_WIDTH_SUMMARY_FIELDNAMES = [
    "preset",
    "code_profile",
    "matrix_kind",
    "verification_status",
...
def summarize_all(raw_dir: Path, summary_dir: Path) -> None:
    ...
    replication_path = raw_dir / "long_stream_cache_width_replication.csv"
    replication_rows = read_csv(replication_path)
    if replication_rows:
        write_csv(
            summary_dir / "long_stream_cache_width_replication_summary.csv",
            summarize_long_stream_cache_width_rows(replication_rows),
            LONG_STREAM_CACHE_WIDTH_SUMMARY_FIELDNAMES,
        )
